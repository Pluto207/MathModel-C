...
> **📄 [队员部署](给队员的部署说明.md) ｜ [启动 Prompt](docs/队员启动Prompt.md) ｜ [多机协同](docs/多机异构Agent交接规范.md) ｜ [A→C 整合流程](docs/A初稿到C优化整合流程.md) ｜ [赛前规则](docs/比赛前规则核对清单.md)**
...
数模A：快速完整初稿（全部 DRAFT）
        ↓ 初稿冻结
数模C：建模主复核 → MATLAB主重做 → 论文主证据化重写
        ↓
C-approved 模型 / 结果 / 论文
```

支持三台电脑、OpenClaw + Claude Code + 其他 Agent；在一起时用局域网共享项目，分开时用受控离线交接包。GitHub 只同步 Skill，不同步赛题项目。
